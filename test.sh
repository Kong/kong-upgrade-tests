...
run_gojira() {
  ...
  local args=$@
  gojira $action -v ${GOJIRA_SETTINGS[*]} --repo $repo -t $version --host $host $@
}
...
install_kong() {
    ...
    if ! [[ "$rebuild" = "1" ]]; then
      image=$(gojira snapshot? --repo $repo -t $version)

      if [[ ! -z $image ]]; then
          msg "Found dev build image $image, not going to install"
          return
      fi
      image=$(gojira image? --repo $repo -t $version)
    fi
    ...
    if [[ -z $image ]]; then
        ...
        gojira build -v --repo $repo -t $version
        image=$(gojira image -v --repo $repo -t $version)
    fi

    gojira up --repo $repo -t $version --alone --image $image
    gojira run --repo $repo -t $version make dev || exit 1
    gojira snapshot --repo $repo -t $version
    gojira down --repo $repo -t $version
}
